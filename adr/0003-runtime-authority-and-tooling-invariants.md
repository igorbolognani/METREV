...
- Prisma 6.19.x in this repository still needs `url = env("DATABASE_URL")` in `packages/database/prisma/schema.prisma`, while editor diagnostics can suggest a Prisma 7-only datasource posture
...
- Prisma stays on the currently validated 6.19.x posture in this repository: `schema.prisma` keeps `url = env("DATABASE_URL")`, `prisma.config.ts` keeps runtime and direct migration URLs aligned, and migration commands continue to run through `packages/database/scripts/run-prisma-with-direct-url.mjs`
...
- Prisma runtime and migration behavior stay aligned with the currently validated command path.
...
- Editor diagnostics for Prisma must be treated as suspect until they agree with command-backed validation for the pinned repository version.
