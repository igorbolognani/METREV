...
export async function fetchResearchWarehouseEligibility(
  ...
  const request = researchWarehouseEligibilityRequestSchema.parse({
    dry_run: true,
    ...payload,
  });
  const params = new URLSearchParams({
    dry_run: String(request.dry_run),
    include_items: String(request.include_items),
    limit: String(request.limit),
  });
  const response = await fetch(
    `${apiBaseUrl}/api/research/warehouse-eligibility?${params.toString()}`,
    {
      cache: 'no-store',
      credentials: 'include',
    },
  ...
  return parseJson(
    response,
    researchWarehouseEligibilityResponseSchema,
    'research warehouse eligibility response',
  );
}

export async function runResearchWarehouseEligibilitySweep(
  payload: ResearchWarehouseEligibilityRequest,
): Promise<ResearchWarehouseEligibilityResponse> {
  const response = await fetch(
    `${apiBaseUrl}/api/research/warehouse-eligibility/sweep`,
    {
      method: 'POST',
      credentials: 'include',
      headers: {
      ...
      body: toJsonBody(researchWarehouseEligibilityRequestSchema, payload),
    },
  );
