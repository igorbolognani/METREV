...
export function buildDashboardWorkspace(input: {
  ...
  return dashboardWorkspaceResponseSchema.parse({
    ...
    evidence_traceability: {
      accepted_refs_count:
        input.latestEvaluation?.normalized_case.evidence_refs.length ?? 0,
      decision_context_available: Boolean(
        input.latestEvaluation?.evidence_decision_context,
      ),
      source_refs_count:
        input.latestEvaluation?.evidence_decision_context?.source_refs.length ??
        0,
      excluded_evidence_count:
        input.latestEvaluation?.evidence_decision_context
          ?.excluded_evidence_summary.length ?? 0,
    },
    hero: {
    ...
    evidence_intelligence: input.evidenceIntelligence,
