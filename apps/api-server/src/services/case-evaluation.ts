...
import { EvidenceDecisionContextBuilder } from './evidence-decision-context-builder';
...
export async function createPersistedCaseEvaluation(
  ...
  const evaluation = await withSpan(
    ...
    async () => {
      ...
      const benchmarkSlice = await withSpan(
        ...
        () =>
          input.evaluationRepository.getEvidenceBenchmarkSlice({
            ...
            componentTypes: evidenceFilters.componentTypes,
            limit: 12,
            materials: evidenceFilters.materials,
            metricTypes: evidenceFilters.metricTypes,
            systemType: evidenceFilters.systemType,
          }),
        {
          case_id: normalizedCase.case_id,
          technology_family: normalizedCase.technology_family,
      ...
      const evidenceDecisionContext = evidenceContextBuilder.build({
        normalizedCase,
        benchmarkSlice,
        componentTypes: evidenceFilters.componentTypes,
        materials: evidenceFilters.materials,
      ...
      const evidenceBenchmarkObservations = [
        evidenceContextBuilder.buildObservation({
