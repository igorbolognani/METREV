...
- `pnpm run validate:db` is the promoted Postgres migration and persistence gate. It expects `DATABASE_URL` and `DIRECT_URL` to point at a disposable PostgreSQL database, applies committed Prisma migrations, and then runs the Postgres-backed Vitest suite.
...
- CI references `METREV_CI_AUTH_SECRET`, `METREV_LLM_API_KEY`, and `OLLAMA_API_KEY` through GitHub Secrets and `METREV_UNPAYWALL_EMAIL` through GitHub Variables. Validation keeps stub LLM mode by default, so those optional provider credentials can remain unset without blocking deterministic checks.
- `METREV_RESEARCH_WORKER_EVIDENCE_DISCOVERY_AUTO_RUN=false` is the local-first default. It keeps the research worker healthy while preventing a fresh quality audit from automatically staging new provider records. Use `true` only for intentional evidence-discovery expansion runs.
